...
describe('Household edit — flow integration (rename, roles, members, invites, leave)', () => {
  ...
  it('admin sets the household location → PUT trimmed value', async () => {
    ...
    await act(async () => {
      fireEvent.press(utils.getByTestId('household-save-location'));
    });

    expect(setSetting).toHaveBeenCalledWith('hh-1', 'household.location', 'Springfield, IL 62704');
  });

  // Typing alone must not write — the Save button is the only committer, same
  // as the household name field.
  it('editing the location without pressing Save writes nothing', async () => {
    const setSetting = setHouseholdSetting as jest.Mock;
    setSetting.mockClear();
    const utils = renderScreen();
    const input = await utils.findByTestId('household-location-input');
  ...
  it('leaving the location untouched keeps Save disabled and writes nothing', async () => {
    const setSetting = setHouseholdSetting as jest.Mock;
    setSetting.mockClear();
    const utils = renderScreen();
    const save = await utils.findByTestId('household-save-location');
